...
{
	char *argv[MAXARGS], *t;
	int argc, c, i, r, p[2], fd, rightpipe, childid;
	int fdnum;
	rightpipe = 0;
	gettoken(s, 0);
again:
	...
	for (;;)
	{
		...
		case '<':
			...
			{
				user_panic("file open error int runcmd <\n");
			}
			fdnum = r;
			if ((r = dup(fdnum, 0)) < 0)
			{
				user_panic("dup error int runcmd <\n");
			}
			close(fdnum);
		...
		case '|':
			...
			if (pipe(p) < 0)
			{
				user_panic("pipe error in runcmd |\n");
			}
			if ((childid = fork()) == 0)
			{
				if (dup(p[0], 0) < 0)
					user_panic("dup error in runcmd | fork==0\n");
				close(p[0]);
				close(p[1]);
				goto again;
			}
